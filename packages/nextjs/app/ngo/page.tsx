    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {!isCreator && (
        <div className="alert alert-warning mb-6">
          Your wallet is not a registered Project Creator. You can still donate and browse projects.
        </div>
      )}
      ...
      <section className="bg-base-100 rounded-xl p-6 shadow-md mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <input
            className="input input-bordered"
            placeholder="Goal in BNB"
            value={goal}
            onChange={e => setGoal(e.target.value)}
          />
          <input
            className="input input-bordered"
            placeholder="Metadata bytes32"
            value={meta}
            onChange={e => setMeta(e.target.value as `0x${string}`)}
          />
        </div>
        <div className="mt-4">
          <button
          ...
          <Link href="/projects" className="btn ml-2">
            View Projects
          </Link>
        </div>
      </section>
