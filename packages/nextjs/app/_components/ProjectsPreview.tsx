...
export default function ProjectsPreview() {
  ...
  const displayProjects = Array.from({ length: Math.min(cardsPerPage, len) }, (_, idx) =>
    MOCK_PROJECTS[(offset + idx) % len]
  );
    ...
    >
      <div className="container mx-auto">
        <div className="mb-7 flex items-start justify-between">
          <div>
            ...
            <p className="text-white/70 text-xs sm:text-sm max-w-xl">
              Every project here fights for something simple: access to water. Choose a cause, fund it, and watch your impact ripple across continents.
            </p>
          </div>
        ...
        <div className="grid grid-cols-5 gap-6 mb-6">
          <div className={`col-span-2 transition-all duration-300 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
            <FadeInSection>
              <Link href={`/projects/${displayProjects[0]?.id}`} className="group block outline-none focus-visible:ring-2 ring-[#E1FFFF]">
                <ProjectCard project={displayProjects[0]} />
              </Link>
            </FadeInSection>
          </div>
          <div className={`col-span-3 transition-all duration-300 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
            <FadeInSection>
              <Link href={`/projects/${displayProjects[1]?.id}`} className="group block outline-none focus-visible:ring-2 ring-[#E1FFFF]">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className={`transition-all duration-300 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
            <FadeInSection>
              <Link href={`/projects/${displayProjects[2]?.id}`} className="group block w-full outline-none focus-visible:ring-2 ring-[#E1FFFF]">
          ...
          <div className={`transition-all duration-300 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
            <FadeInSection>
              <Link href={`/projects/${displayProjects[3]?.id}`} className="group block w-full outline-none focus-visible:ring-2 ring-[#E1FFFF]">
          ...
          <div className={`transition-all duration-300 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
            <FadeInSection>
              <Link href={`/projects/${displayProjects[4]?.id}`} className="group block w-full outline-none focus-visible:ring-2 ring-[#E1FFFF]">
