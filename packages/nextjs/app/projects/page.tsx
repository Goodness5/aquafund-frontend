...
function CreateProjectForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-8 p-4 bg-base-200 rounded-xl shadow flex flex-col gap-4 max-w-xl mx-auto">
...
export default function ProjectsPage() {
	...
	const cards = useMemo(() => (ids || []).map(id => Number(id)), [ids]);
...
function ProjectCard({ projectId }: { projectId: number }) {
	...
	return (
		<div className="rounded-xl bg-base-100 shadow-md p-4 flex flex-col gap-3">
			...
			<div className="text-sm opacity-80">Raised: {formatEther(raised)} / {formatEther(goal)} BNB</div>
